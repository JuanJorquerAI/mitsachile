...
- `scripts/` — shell scripts operativos (ver su README): `setup-wp.sh` (bootstrap WP local), `provision.sh` (páginas/menús idempotente), `wpcli.sh` (wrapper WP-CLI), `aplicar-seo-produccion.sh` (sube robots/sitemap por SFTP, requiere `MITSA_SFTP_*` en `.env`), `export-estatico.sh` (espejo estático a `export/`).
